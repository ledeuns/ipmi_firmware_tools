...
class FirmwareImage:
	...
	correct_signature = struct.unpack('<I',b'\x9f\xff\xff\xa0')[0]
	...
	def __init__(self):
		...
		self.signature = struct.unpack('<I',b'\x9f\xff\xff\xa0')[0]
		self.type = 0
	...
	def getRawString(self):
		contents = struct.pack(b'<5I16s4I',self.imagenum, self.base_address, self.length, self.load_address, self.exec_address, self.name, self.image_checksum, self.signature, self.type, self.footer_checksum)
	...
	def computeFooterChecksum(self):
		footer_data = struct.pack(b'<5I16s3I',self.imagenum, self.base_address, self.length, self.load_address, self.exec_address, self.name, self.image_checksum, self.signature, self.type)
